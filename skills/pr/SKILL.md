...
unavailable. Do not create a comment solely to manufacture review provenance.
...
and the NUL-safe diff to that head contains only `evidence.md`, `decisions.md`,
`history.jsonl`, or `status.yml` in the reviewed work item. Any other changed
path makes it stale. Only a current receipt plus green required CI transitions
