...
verifier's persisted PASS receipt for the independently derived repository,
work ID, baseline/reviewed heads, canonical scope and no-renames diff digests,
expected subject, reviewer identity and role, checks, artifact content digest,
issuer provenance, and evidence reference; the verifier must not be the
...
of issuer reference, every derived subject field, reviewer principal and role,
implementer flag, session or dispatch, checks, verdict, and artifact reference
and digest. Fail closed on unavailable or mismatched resolution. Do
...
`status.yml` field-by-field; only the policy's lifecycle/CI fields plus a
grammar-valid `change_request` are neutral. Invalid status YAML fails closed,
and a risk, identity, work-type, or review-loop change makes the receipt
