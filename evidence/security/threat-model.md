...
| Configured argv names Git, Forge, or infrastructure authority | Empty and whitespace-ambiguous tokens fail. Every argv token is checked by basename for `git`, `gh`, `glab`, and `terraform`; the explicit read-only exception list is empty. The separate blocked-launcher list is illustrative, not exhaustive. | Portable schema tests cover authority-bearing tokens in first and later positions, path-qualified tokens, shell syntax, and accepted repository-script residuals. The naming predicate is not evidence of arbitrary program semantics. |
...
| An external alternate object store changes | The in-tree alternates declaration is bound, but Flow42 does not resolve or snapshot the external store. An external object is inert for integration until a separately bound ref, `HEAD`, index, or worktree surface references it. | The declaration changes the snapshot; an external-only object does not; adding a ref to that object does. |
